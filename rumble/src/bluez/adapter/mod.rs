...
pub type AdvHandler = Box<dyn Fn(hci::LEAdvertisingInfo) + Send>;
pub use bluez::protocol::hci::{LEAdvertisingData, LEAdvertisingInfo};
...
pub struct ConnectedAdapter {
    ...
    advertisement_handler: Arc<Mutex<Option<AdvHandler>>>,
}
...
impl ConnectedAdapter {
    pub fn new(adapter: &Adapter) -> Result<ConnectedAdapter> {
        ...
        let connected = ConnectedAdapter {
            ...
            advertisement_handler: Arc::new(Mutex::new(None)),
    ...
    pub fn on_advertisement(&self, new_handler: AdvHandler) {
        let handler = self.advertisement_handler.clone();
